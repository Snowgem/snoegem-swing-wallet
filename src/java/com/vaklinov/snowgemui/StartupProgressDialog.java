...
// Taken from repository https://github.com/zlatinb/snowgem-swing-wallet-ui under an MIT license
...
public class StartupProgressDialog extends JFrame {
    ...
    private final SnowgemClientCaller clientCaller;
    
    public StartupProgressDialog(SnowgemClientCaller clientCaller) 
    {
        this.clientCaller = clientCaller;
        
        URL iconUrl = this.getClass().getClassLoader().getResource("images/snowgem-logo-large.png");
		...
		JLabel snowgemWalletLabel = new JLabel(
			"<html><span style=\"font-style:italic;font-weight:bold;font-size:2.65em\">" + 
		    "Snowgem<span style=\"font-style:italic;font-weight:bold;font-size:1.5em;vertical-align:super\">" + 
		    "\u00AE</span> Wallet</span></html>");
		snowgemWalletLabel.setBorder(BorderFactory.createEmptyBorder(16, 16, 16, 16));
		// todo - place in a panel with flow center
		JPanel tempPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 1, 1));
		tempPanel.add(snowgemWalletLabel);
...
//        }
        ...
        Log.info("Splash: checking if snowgemd is already running...");
        boolean shouldStartSnowgemd = false;
        try {
        	...
        	{
        		shouldStartSnowgemd = true;
        	}
        }
        
        if (!shouldStartSnowgemd) {
        	Log.info("Splash: snowgemd already running...");
            // What if started by hand but taking long to initialize???
//            doDispose();
//            return;
        } else
        {
        	Log.info("Splash: snowgemd will be started...");
        }
        
        final Process daemonProcess = 
        	shouldStartSnowgemd ? clientCaller.startDaemon() : null;
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
            	Log.info("Stopping snowgemd because we started it - now it is alive: " + 
                ...
                try 
                {
	                ...
	                while (!StartupProgressDialog.this.waitFor(daemonProcess, 3000))
	                {
	                	...
	                	Log.info("Waiting for " + ((end - start) / 1000) + " seconds for snowgemd to exit...");
	                ...
	                if (StartupProgressDialog.this.isAlive(daemonProcess)) {
	                	Log.info("snowgemd is still alive although we tried to stop it. " +
	                                           "Hopefully it will stop later!");
	                        //System.out.println("snowgemd is still alive, killing forcefully");
	                        //daemonProcess.destroyForcibly();
	                    } else
	                    	Log.info("snowgemd shut down successfully");
                } catch (Exception bad) {
                	Log.error("Couldn't stop snowgemd!", bad);
                }
            }
        });
    ...
    private void performOSXBundleLaunch() throws IOException, InterruptedException {
        ...
        File bundlePath = new File(System.getProperty("snowgem.location.dir"));
